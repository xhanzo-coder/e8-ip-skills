...
先从 [风格注册表](references/style-registry.json) 解析风格能力、成熟度、参考图和专属校验器，再只读取选中的风格定义。不得向用户承诺注册表中列为 `unverified_outputs` 的批量产物；首次只能生成单张候选验证。
...
3. 创建人物但尚未选择风格时，先展示三个内置风格的真实目录预览并给出一个有理由的推荐，随后停下等待用户选择或授权推荐。目录选择只是确定画法，不等于授权生图。
...
10. 执行注册表声明的专属运行校验器。黑白或像素正面母图必须在烧图前通过 `--phase pre`，生成后通过 `--phase post`；任一硬门失败即拒绝。
